...
namespace ocl {
    ...
    inline
    void
    bind_non_buffer_args(const impl::ck_body& t,
    ...
    inline
    void
    bind_buffer_args(const impl::ck_body& r, unsigned& buf_num,
                     be::kernel& k) {
    }
#endif
    ...
    namespace impl {
        ...
        {
            auto tail=custom_kernel_args(ax...);
            expr<dop::custom_k_arg<dvec<_T> >, _A0, typeof(tail)> e(a0, tail);
            return e;
        }
    }
    ...
    custom_kernel(const std::string& name,
                  ...
                  const _AX& ... ax)
    {
        auto e0=impl::custom_kernel_args(ax...);
...
ocl::test::test_custom_kernel()
{
    ...
    "    }\n"
    "}\n";
    const char* kname="add";

    dvec<float> v2(8, 2.4f);
    auto ck=custom_kernel<float>(kname, kbody, 1.5f);
    std::cout << be::demangle(typeid(ck).name()) << std::endl;
    v2 = ck;
}
